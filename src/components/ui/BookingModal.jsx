...
import { CalendarDays, CheckCircle, FileText, Loader2, Send, X } from 'lucide-react';
import axios from '../../lib/axios';
import { useSelector } from 'react-redux';
import { useNavigate } from 'react-router-dom';

export default function BookingModal({ isOpen, onClose, gig, mode = 'contact' }) {
    ...
    const handleSubmit = async () => {
        ...
        if (isBooking && !scheduledAt) {
            setError('Veuillez choisir une date pour votre reservation.');
            return;
        }

        if (!notes.trim()) {
            setError(isBooking ? 'Veuillez ajouter un message pour l expert.' : 'Veuillez decrire votre probleme avant d envoyer.');
            return;
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 relative">
                <div className="bg-gray-50 border-b border-gray-100 p-6 flex items-center justify-between">
                    <h2 className="text-xl font-bold text-gray-900">{isBooking ? 'Reservation du service' : 'Demande de contact'}</h2>
                    ...
                    {step === 1 ? (
                        <div>
                            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 mb-8 flex gap-4 items-center">
                                ...
                                <div>
                                    <p className="text-xs text-blue-600 font-bold uppercase tracking-wider mb-0.5">{isBooking ? 'Vous reservez' : 'Vous contactez'}</p>
                            ...
                            <div className="mb-6">
                                <h3 className="text-xl font-bold text-gray-900 mb-2">{isBooking ? 'Quand souhaitez-vous reserver ?' : 'Quel est votre besoin ?'}</h3>
                                <p className="text-gray-500 text-sm">
                                    {isBooking
                                        ? 'Choisissez une date souhaitee et ajoutez un message. L expert pourra accepter ou ajuster la demande.'
                                        : 'Decrivez votre probleme a l artisan. Vous pourrez convenir d une date et d un tarif ensemble apres ce premier contact.'}
                                </p>
                            </div>
                            ...
                            <div className="space-y-4">
                                {isBooking && (
                                    <div className="p-4 border-2 border-gray-200 focus-within:border-blue-500 rounded-2xl transition-colors bg-white shadow-sm">
                                        <label className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                                            <CalendarDays size={16} /> Date souhaitee
                                        </label>
                                        <input
                                            type="datetime-local"
                                            value={scheduledAt}
                                ...
                                <div className="p-4 border-2 border-gray-200 focus-within:border-blue-500 rounded-2xl transition-colors bg-white shadow-sm">
                                    ...
                                    <textarea
                                        ...
                                        placeholder={isBooking ? "Ex: Bonjour, je souhaite reserver ce service pour..." : "Ex: Bonjour, j'ai besoin de vos services pour..."}
                            ...
                            <div className="mt-8 flex gap-3">
                                ...
                                <button onClick={handleSubmit} disabled={loading} className="flex-1 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-500/30 group disabled:opacity-70 disabled:cursor-not-allowed">
                                    {loading ? <Loader2 size={20} className="animate-spin" /> : (isBooking ? 'Reserver' : 'Envoyer la demande')}
                        ...
                        <div className="flex flex-col items-center justify-center text-center py-6">
                            ...
                            <h3 className="text-3xl font-extrabold text-gray-900 mb-4">{isBooking ? 'Reservation envoyee !' : 'Message envoye !'}</h3>
                            <p className="text-gray-500 text-lg mb-8 leading-relaxed">
                                {isBooking ? 'Votre reservation a ete envoyee avec succes.' : 'Votre demande a ete envoyee avec succes.'} <strong>{gig.user?.name}</strong> prendra connaissance de votre message et vous contactera tres prochainement.
                            </p>
